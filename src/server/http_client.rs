...
use super::http::{Body, OnResponseResult, OnResponse};
...
impl Msg {
    ...
    fn on_response(&mut self, res: Response) -> Next {
        if *res.status() != StatusCode::Ok {
            ...
            return self.on_end(Err(box_err!("{}", res.status())));
        }
...
impl<H: HyperTransport> HyperHandler<H> for Route {
    fn on_request(&mut self, req: &mut Request) -> Next {
        debug!("{:?} send request {}", self, req.uri());
    ...
    fn on_request_writable(&mut self, encoder: &mut Encoder<H>) -> Next {
        debug!("{:?} on request writable", self);
        match *self {
    ...
    fn on_response_readable(&mut self, decoder: &mut Decoder<H>) -> Next {
        debug!("{:?} on response readable", self);
...
impl Client {
    pub fn new() -> Result<Client> {
        ...
        let c = try!(HyperClient::<Route>::configure()
    ...
    fn request(&self, url: Url, r: Route) -> Result<()> {
        if let Err(e) = self.c.request(url, r) {
    ...
    pub fn post_message(&self, url: Url, msg: msgpb::Message, cb: OnResponse) -> Result<()> {
        ...
        self.request(url, Route::Msg { msg: pm })
    }
