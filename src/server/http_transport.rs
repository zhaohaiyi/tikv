...
use pd::PdClient;
use raftstore::store::{SendCh, Msg as StoreMsg, Transport};
...
pub struct StoreAddrPeeker<T: PdClient> {
    cluster_id: u64,
    pd_client: Arc<RwLock<T>>,
    store_addrs: HashMap<u64, String>,
}

impl<T: PdClient> StoreAddrPeeker<T> {
    pub fn new(cluster_id: u64, pd_client: Arc<RwLock<T>>) -> StoreAddrPeeker<T> {
        StoreAddrPeeker {
...
struct StorePending {
    pub url: Url,
    pub is_posting: bool,
    pub msgs: VecDeque<(Message, SendCh)>,
}
...
struct Worker<T: PdClient> {
    client: Client,

    store_peeker: StoreAddrPeeker<T>,
...
impl<T: PdClient> Worker<T> {
    pub fn new(client: Client, peeker: StoreAddrPeeker<T>) -> Worker<T> {
    ...
    pub fn run(&mut self) {
        loop {
            ...
            match task {
                Task::Msg { msg, ch } => self.handle_msg(msg, ch),
                Task::OnMsgResp { store_id } => self.handle_on_msg_resp(store_id),
                Task::Snapshot { msg, ch } => self.handle_snapshot(msg, ch),
                Task::Quit => return,
            }
        }
    }
    ...
    #[allow(map_entry)]
    fn handle_msg(&mut self, msg: Message, ch: SendCh) {
        ...
        if !self.store_pendings.contains_key(&to_store_id) {
            ...
            let addr = match self.store_peeker.get_address(to_store_id) {
                Err(e) => {
                    ...
                    report_unreachable(ch, region_id, to_store_id);
                    return;
                }
                Ok(addr) => addr,
            };
...

        {
            ...
            if !store_pending.msgs.is_empty() || store_pending.is_posting {
                store_pending.msgs.push_back((msg, ch));
                return;
            }
        }
        ...
        self.post_msg(to_store_id, msg, ch);
    }

    fn handle_snapshot(&mut self, msg: Message, ch: SendCh) {
        let to_store_id = msg.get_raft().get_message().get_to();
        let region_id = msg.get_raft().get_region_id();
        ...
        self.client
            .post_message(url,
                          ...
                          box move |resp: OnResponseResult| {
                              ...
                              report_snapshot(ch, region_id, to_store_id, status);
                          })
            .unwrap();
    }
    ...
    fn handle_on_msg_resp(&mut self, store_id: u64) {
        ...
        let (msg, ch) = pending.unwrap();
        self.post_msg(store_id, msg, ch);
    }


    fn post_msg(&mut self, store_id: u64, msg: Message, ch: SendCh) {
        let url = {
            let mut store_pending = self.store_pendings.get_mut(&store_id).unwrap();
        ...
        self.client
            .post_message(url,
                          ...
                          box move |resp: OnResponseResult| {
                              if resp.is_err() {
                                  report_unreachable(ch, region_id, store_id);
                              }
...
fn report_snapshot(ch: SendCh, region_id: u64, to_store_id: u64, status: SnapshotStatus) {
    if let Err(e) = ch.send(StoreMsg::ReportSnapshot {
        region_id: region_id,
        to_store_id: to_store_id,
        status: status,
    }) {
        error!("report snapshot to store {} with region {} err {:?}",
               to_store_id,
               region_id,
               e);
    }
...
fn report_unreachable(ch: SendCh, region_id: u64, to_store_id: u64) {
    if let Err(e) = ch.send(StoreMsg::ReportUnreachable {
        region_id: region_id,
        to_store_id: to_store_id,
    }) {
        error!("report unreachable to store {} with region {} err {:?}",
               to_store_id,
               region_id,
               e);
    }
...
impl HttpTransport {
    pub fn new<T>(cluster_id: u64,
                  client: Client,
                  pd_client: Arc<RwLock<T>>)
                  -> Result<HttpTransport>
        where T: PdClient + 'static
    {
        let peeker = StoreAddrPeeker::new(cluster_id, pd_client);

        let mut worker = Worker::new(client, peeker);
...
impl Transport for HttpTransport {
    fn send(&self, req: RaftMessage, ch: SendCh) -> RaftStoreResult<()> {
        ...
        if let Err(e) = {
            if is_snapshot {
                tx.send(Task::Snapshot { msg: msg, ch: ch })
            } else {
                tx.send(Task::Msg { msg: msg, ch: ch })
            }
        } {
